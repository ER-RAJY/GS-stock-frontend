    ...
    <div class="space-y-6">
      ...
      @if (loading()) {
        ...
        @if (authService.isAdmin()) {
          ...
          <!-- Alert count for ADMIN -->
          <div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-gray-600">Alertes de stock</p>
                <p class="text-2xl font-bold text-gray-900 mt-2">
                  {{ alertsCount() }}
                </p>
              </div>
        ...
        <!-- COMPTABLE Dashboard: Stats widgets -->
        @if (authService.isComptable()) {
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-gray-600">Nombre de produits</p>
                  <p class="text-2xl font-bold text-gray-900 mt-2">
                    {{ productsCount() }}
                  </p>
                </div>
        ...
        <!-- Top Sorties Widget (both ADMIN and COMPTABLE) -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">Top 5 produits par sorties</h2>
          @if (topSortiesLoading()) {
            <div class="text-center py-4 text-gray-500">Chargement...</div>
          } @else if (topSorties().length === 0) {
            <div class="text-center py-4 text-gray-500">Aucune donnée disponible</div>
          } @else {
            <div class="space-y-3">
              @for (item of topSorties(); track item.produitId) {
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div class="flex-1">
                    <p class="text-sm font-medium text-gray-900">{{ item.designation }}</p>
                    <p class="text-xs text-gray-500">ID: {{ item.produitId }}</p>
                  </div>
                  <div class="text-right">
                    <p class="text-sm font-bold text-gray-900">{{ item.totalSortie }}</p>
